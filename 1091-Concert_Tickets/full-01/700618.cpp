...
// Including seg_tree_order_statistics_node_update
#include <stdio.h>
#include <cassert>
using namespace std;
...
ordered_set;
//*X.find_by_order(1), X.order_of_key(-5)(count smaller)
...
int main(){
    ...
    REP(0,m){
        //for(auto pa : se)cout<<pa<<" ";
        //cout<<endl;
        ll pa = {t[i],LLONG_MAX};
        lo c = se.order_of_key(pa);
        //cout<<pa<<" "<<c<<endl;
        if(c==0){
            cout<<"-1"<<endl;
        }else{
            auto it = se.find_by_order(c-1);
            cout<<(*it).X<<endl;
            se.erase(it);
        }
    }
